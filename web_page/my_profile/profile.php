...
if(isset($user_data['user_id'])){
    ...
    $select = mysqli_query($con, "SELECT * FROM users WHERE user_id = '$id' ") or die(mysqli_error($con));
...
if (isset($_POST['submit'])) {
    ...
    $sql = mysqli_query($con, "UPDATE users SET nama_lengkap='$nama_lengkap', bank='$bank', no_akun='$no_akun', nm_akun='$nm_akun', ovo='$ovo', gopay='$gopay', dana='$dana' WHERE user_id='$id'");

    if ($sql) {
        echo '<script>alert("Berhasil Mengubah Profile."); document.location="index_data.php?page=my_profile";</script>';
    }else{
        echo '<script>alert("Gagal Memproses Update Profile."); document.location="index_data.php?page=my_profile";</script>';
    }
...
if (isset($_POST['change-passwd'])){
    if ($_POST['newpassword'] == $_POST['newpassword2']) {
        ...
        $sql = mysqli_query($con, "UPDATE users SET password='$newpasswd' WHERE user_id='$id'");

        if ($sql) {
            echo '<script>alert("Berhasil Mengganti Password."); document.location="index_data.php?page=my_profile";</script>';
            
        }else {
            echo '<script>alert("Gagal Mengganti Password."); document.location="index_data.php?page=my_profile";</script>';
