...
if(isset($_POST["item_name"]))
{
 ...
 for($count = 0; $count<count($item_name); $count++)
 {
  ...
  if($item_name_clean != '' && $item_code_clean != '' && $item_desc_clean != '' && $item_price_clean != '' &&  $item_joki_clean != '' &&  $item_barang_clean != '' && $item_harga_clean != '')
  {
   $query .= '
   INSERT INTO hasiltembak(joki,nmalamat,merchant,barang,jumlah,harga,fee,resi) 
   VALUES("'.$item_name_clean.'", "'.$item_code_clean.'", "'.$item_desc_clean.'", "'.$item_price_clean.'", "'.$item_joki_clean.'", "'.$item_barang_clean.'","0","'.$item_harga_clean.'"); 
   ';
  }
 }
 if($query != ''){
